    ...
    for i in json_response:
        for attribute, value in i.items():
            for val in value:
                ...
                if val['email'] == email:
                    if val['password'] == "":
                        col.update({"_id": id, "users.email": email}, {"$set": {"users.$.password": password, "users.$.username": username}})
                        return "password and username updated"
                        msg = "done"
                    else:
                        return "this email id is already registered."
                        msg = "done"
    if msg == "":
        return "not a valid your for registration"
